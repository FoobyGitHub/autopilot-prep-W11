...
**`-ForceDrivers`** — use this when running `-PrepUSB` or `-PatchISO` on a different machine to the one being built. Driver detection reads the hardware on the machine running the script — if that's not the target, detection won't match and drivers will be skipped. `-ForceDrivers` bypasses detection and injects all driver sets (VMD, Wi-Fi/BT) regardless. For cross-machine prep this should be the default:
...
Use `-ForceDrivers` when prepping on a different machine to the target — detection reads the local CPU and won't see the target's hardware.
